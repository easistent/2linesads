...
/*jslint regexp: true, sloppy: true, vars: true, fragment: false, browser: true, indent: 4, maxlen: 256 */
/*global ActiveXObject, window, document, alert, unescape*/
...
function addCss(sel, css) {
    ...
    var styl = document.styleSheets[document.styleSheets.length - 1];
...
function highlightSearchTerms(searchText, bannerTitle, bannerDesc, bannerURL, bannerPicture, userID) {
    ...
    if (document.getElementById("body_content")) {
        ...
        if (searchText.indexOf("|") > 0) {
            ...
            for (xx = 0; xx < searchLoop.length; xx++) {
...
// This "draws" the ad - Desktop
function dotxtadMouse(adID, adPicture, adTitle, adText, adURL, adword, userID) {
    // create DIV
...
function dotxtadTouch(adID, adPicture, adTitle, adText, adURL, adword, userID) {
    alert('ciao!');
}

function dotxtad(adID, adPicture, adTitle, adText, adURL, adword, userID) {
    if (isTouchDevice() === false) {
        dotxtadMouse(adID, adPicture, adTitle, adText, adURL, adword, userID);
    } else {
        dotxtadTouch(adID, adPicture, adTitle, adText, adURL, adword, userID);
    }
}
